...
class MatDataLoader(Dataset):
    def __init__(self, data_length, base_dir):
        ...
        self.theta =  torch.cat([self.theta_1_dot, self.theta_2_dot, self.theta_3_dot], dim = 1)                                                                                           # shape: batch_size x 3
        ...
        self.s =  torch.cat([self.x, self.y, self.z], dim = 1)                                                                                                                             # shape: batch_size x 3
        ...
        self.y_Ddot = torch.tensor(h5py.File(os.path.join(self.base_dir, "simulated_x_Ddot.mat"), "r")["data_x_Ddot"][3:data_length, 1]).unsqueeze(1)  
        self.z_Ddot = torch.tensor(h5py.File(os.path.join(self.base_dir, "simulated_x_Ddot.mat"), "r")["data_x_Ddot"][3:data_length, 1]).unsqueeze(1)  
        self.s_Ddot =  torch.cat([self.x_Ddot, self.y_Ddot, self.z_Ddot], dim = 1)                                                                                                         # shape: batch_size x 3
        
        self.tau_1 = torch.tensor(scipy.io.loadmat(os.path.join(self.base_dir, "tau_1.mat"))["tau_first_row"][0, 3:data_length]).unsqueeze(1)
        self.tau_2 = torch.tensor(scipy.io.loadmat(os.path.join(self.base_dir, "tau_2.mat"))["tau_second_row"][0, 3:data_length]).unsqueeze(1)
        self.tau_3 = torch.tensor(scipy.io.loadmat(os.path.join(self.base_dir, "tau_3.mat"))["tau_third_row"][0, 3:data_length]).unsqueeze(1)
